...
public class ProductSkuPriceView extends JDialog {
    ...
    public ProductSkuPriceView(JDialog parent, ProductSku currentSku) {
        ...
        super(parent, "Price Management - SKU: " + currentSku.getSkuCode(), true);
    ...
    private void initUI() {
        ...
        inputPanel.add(new JLabel("Price ID (Empty when adding):"));
        ...
        btnAdd = new JButton("Add Price");
        btnEdit = new JButton("Edit Price");
        btnDelete = new JButton("Delete Price");
    ...
    public ProductSkuPrice getPriceFromForm() {
        ...
        try {
            ...
            if (!txtId.getText().trim().isEmpty()) {
                price.setId(new BigInteger(txtId.getText().trim()));
            }

            price.setPrice(new BigDecimal(txtPrice.getText().trim()));

            if (!txtOriginalPrice.getText().trim().isEmpty()) {
                price.setOriginalPrice(new BigDecimal(txtOriginalPrice.getText().trim()));
            }

            price.setCurrency(txtCurrency.getText().trim());

            price.setActive(chkActive.isSelected());

            price.setProductSku(currentSku);

            return price;

        } catch (NumberFormatException e) {

            showMessage("Price and Original Price must be numeric values!");
    ...
    public void updateRow(int rowIndex, ProductSkuPrice p) {
        ...
        tableModel.setValueAt(p.isActive() ? "Yes" : "No", rowIndex, 4);
    }
    ...
    public int getSelectedRow() {
        return priceTable.getSelectedRow();
    }
    ...
    public BigInteger getSelectedId() {
        ...
        if (row >= 0) {
            return new BigInteger(priceTable.getValueAt(row, 0).toString());
        }
